...
func (s *Server) registerRoutes(h *hertzserver.Hertz) {
	...
	api := h.Group("/api")
	{
		...
		api.POST("/books/settings/import", apiHandlers.HandleBookSettingsImport)
		api.POST("/books/settings/identify", apiHandlers.HandleBookSettingsIdentify)
		api.GET("/books/settings/export", apiHandlers.HandleBookSettingsExport)
		api.GET("/books/settings/export/groups", apiHandlers.HandleBookSettingsExportGroups)
		...
		api.POST("/skills/drafts", apiHandlers.HandleSkillDraftCreate)
		api.GET("/skills/drafts", apiHandlers.HandleSkillDraftList)
		api.GET("/skills/drafts/:name", apiHandlers.HandleSkillDraftRead)
		api.PUT("/skills/drafts/:name", apiHandlers.HandleSkillDraftUpdate)
		api.DELETE("/skills/drafts/:name", apiHandlers.HandleSkillDraftDiscard)
		api.POST("/skills/drafts/:name/confirm", apiHandlers.HandleSkillDraftConfirm)
		api.POST("/continuation/explore", apiHandlers.HandleContinuationExplore)
		api.GET("/continuation/tasks", apiHandlers.HandleContinuationTasks)
		api.GET("/continuation/tasks/:id/stream", apiHandlers.HandleContinuationTaskStream)
		api.POST("/continuation/tasks/:id/abort", apiHandlers.HandleContinuationTaskAbort)
		api.DELETE("/continuation/tasks/:id", apiHandlers.HandleContinuationTaskDelete)
		api.POST("/continuation/commit", apiHandlers.HandleContinuationCommit)
