...
func (s *Server) registerRoutes(h *hertzserver.Hertz) {
	...
	api := h.Group("/api")
	{
		...
		api.POST("/interactive/opening/roll", apiHandlers.HandleInteractiveOpeningRoll)
		...
		api.GET("/story-memory-structures", apiHandlers.HandleStoryMemoryStructures)
		api.POST("/story-memory-structures", apiHandlers.HandleStoryMemoryStructureCreate)
		api.GET("/story-memory-structures/:id", apiHandlers.HandleStoryMemoryStructure)
		api.PATCH("/story-memory-structures/:id", apiHandlers.HandleStoryMemoryStructureUpdate)
		api.DELETE("/story-memory-structures/:id", apiHandlers.HandleStoryMemoryStructurePresetDelete)
		api.GET("/opening-selectors", apiHandlers.HandleOpeningSelectors)
		api.POST("/opening-selectors", apiHandlers.HandleOpeningSelectorCreate)
		api.GET("/opening-selectors/:id", apiHandlers.HandleOpeningSelector)
		api.PATCH("/opening-selectors/:id", apiHandlers.HandleOpeningSelectorUpdate)
		api.DELETE("/opening-selectors/:id", apiHandlers.HandleOpeningSelectorDelete)
