...
func newInteractiveConversation(store *interactive.Store, denovaDir, workspace, storyID, branchID, user string, replyTargetChars int, cfg *config.Config) *interactiveConversation {
	return &interactiveConversation{store: store, denovaDir: denovaDir, workspace: workspace, cfg: cfg, storyID: storyID, branchID: branchID, user: user, replyTargetChars: replyTargetChars, directorGenerator: generateInteractiveDirector}
}
...
func (c *interactiveConversation) storyDirectorForMeta(meta interactive.StoryMeta) interactive.StoryDirector {
	return loadStoryDirectorForMeta(c.denovaDir, meta)
}

func loadStoryDirectorForMeta(denovaDir string, meta interactive.StoryMeta) interactive.StoryDirector {
	director := loadStoryDirector(denovaDir, meta.StoryDirectorID)
	...
	return interactive.ResolveStoryDirectorModules(denovaDir, director)
}
