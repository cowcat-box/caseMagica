...
var catalogZH = map[string]string{
	...
	"api.access.authRequired":                 "请先输入 CaseMagica 远程访问用户名和密码",
	"api.access.lanDisabled":                  "当前未开启局域网访问",
	"api.books.titleRequired":                 "title 不能为空",
	"api.books.casemagicaDirMissing":                "CaseMagica 数据目录未配置",
	...
	"api.books.exportNoContent":               "没有可导出的内容，请先勾选要导出的内容",
	"api.books.exportTXTChaptersOnly":         "txt 格式仅支持章节正文，其他内容请选择 zip 打包",
	"api.books.settingsImportFileRequired":    "请选择要导入的设定文件",
	"api.books.settingsImportTooLarge":        "设定文件不能超过 64MB",
	"api.books.settingsImportReadFailed":      "读取设定文件失败: {{detail}}",
	"api.books.settingsImportTypeInvalid":     "仅支持导入 md 或 txt 格式的设定文件",
	"api.books.settingsImportKindUnknown":     "无法识别材料类型，请选择导入目标后重试",
	"api.books.settingsImportKindInvalid":     "不支持的导入目标类型",
	"api.books.settingsExportFileRequired":    "请提供要导出的设定文件路径",
	"api.continuation.invalidTarget":          "不支持的提交目标",
	"api.continuation.emptyContent":           "追加内容不能为空",
	...
	"api.skills.invalidScope":                 "scope 必须是 user 或 workspace",
...
var catalogEN = map[string]string{
	...
	"api.access.authRequired":                 "Enter the CaseMagica remote access username and password first.",
	"api.access.lanDisabled":                  "LAN access is not enabled.",
	"api.books.titleRequired":                 "Title is required.",
	"api.books.casemagicaDirMissing":                "CaseMagica data directory is not configured.",
	...
	"api.books.exportNoContent":               "There is nothing to export. Select content first.",
	"api.books.exportTXTChaptersOnly":         "The txt format supports chapter text only; use zip for other content.",
	"api.books.settingsImportFileRequired":    "Select a setting file to import.",
	"api.books.settingsImportTooLarge":        "Setting files cannot exceed 64MB.",
	"api.books.settingsImportReadFailed":      "Failed to read the setting file: {{detail}}",
	"api.books.settingsImportTypeInvalid":     "Only md or txt setting files can be imported.",
	"api.books.settingsImportKindUnknown":     "Could not identify the material type. Choose an import target and retry.",
	"api.books.settingsImportKindInvalid":     "Unsupported import target type.",
	"api.books.settingsExportFileRequired":    "Provide the setting file path to export.",
	"api.continuation.invalidTarget":          "Unsupported commit target.",
	"api.continuation.emptyContent":           "Append content cannot be empty.",
	...
	"api.skills.invalidScope":                 "Scope must be user or workspace.",
