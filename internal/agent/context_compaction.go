...
// context-checkpoint record produced by CaseMagica's compaction pipeline.
...
func buildContextCompactionTranscript(messages []*schema.Message, existingCheckpoint, referenceContext string, sourceTokens, inputChars int, retryInstruction string, policy contextCompactionPolicy) string {
	...
	sb.WriteString("请按系统要求压缩以下 CaseMagica 上下文。基于 existing_checkpoint、reference_context 与 new_context 增量生成新的历史 checkpoint，保留所有会影响后续剧情、任务、关系、世界状态或用户偏好的信息。\n")
