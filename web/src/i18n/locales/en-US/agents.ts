const agents = {
  ...
  'agents.configManager.subtitle': 'Lore, presets, Skills, automation, and story memory management',
  ...
  'agents.field.toolResultKeepRecent': 'Full Results To Keep',
  'agents.field.toolResultBudget': 'Tool Result Budget (KB)',
  'agents.field.toolResultPreview': 'Per-result Preview Chars',
  ...
  'agents.prompt.protectedNote': 'Custom prompts take priority over CaseMagica built-in prompts for behavior, creative preferences, strategy, and style, but they cannot override tool permissions, output protocols, interactive no-write boundaries, structured JSON requirements, or backend validation.',
  'agents.prompt.builtinNote': 'Showing CaseMagica’s built-in default system prompt. Edit it to save a custom System Prompt in the current layer.',
  ...
  'agents.subAgents.newPrompt': 'You are a CaseMagica custom SubAgent. Only handle the task delegated by the parent Agent, keep the result focused and actionable, and do not assume tool capabilities that the parent Agent did not grant.',
  ...
  'agents.context.compactionNote': 'Model profile context window tokens default to 400K. When context usage reaches the threshold, CaseMagica synchronously compacts older history. Before compaction, runtime keeps the current effective transcript without a configurable turn window.',
  'agents.context.compactionTargetNote': 'Configure the output policy for the independent Context Compaction Agent. After compaction, CaseMagica keeps the latest 1 raw turn by default, then compresses older context to 5%-20%. Dense context should use the upper end so key user intent, story changes, and pending work are not over-compressed.',
