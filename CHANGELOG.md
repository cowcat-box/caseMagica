...
- 修复资料库正文编辑器输入后光标跳到文档末尾的问题；修复资料库自动保存的 revision 冲突误报。

#### English

- Fixed the lore editor cursor jumping to the end of the document after typing; fixed the lore autosave revision-conflict false alarm.
...
- Windows 新建或切换书籍时不再因工作区变更存储对 `.casemagica` 目录执行不受支持的同步而失败；账本、内容 blob 和作品文件仍保留完整的文件级持久化同步。
- Creating or switching books on Windows no longer fails when workspace-change storage encounters unsupported directory synchronization under `.casemagica`; ledger, content blob, and manuscript files retain full file-level durability synchronization.
...
- 默认测试不再访问实时搜索引擎；只有显式设置 `CASEMAGICA_LIVE_WEB_SEARCH_TEST=1` 才运行联网聚合检查，保持常规测试快速且可复现。
- The default test suite no longer accesses live search engines; network aggregation runs only with `CASEMAGICA_LIVE_WEB_SEARCH_TEST=1`, keeping normal tests fast and reproducible.
...
- v0.1.18 已发布的 `.denova`/`.casemagica` 工作区发现、故事 JSONL、通用 StateOps、会话消息、资料库和配置读取路径继续保留；自动迁移或状态重写仍使用 revision 校验、备份或可回放事件。
- Read paths remain for v0.1.18 `.denova`/`.casemagica` workspace discovery, story JSONL, generic StateOps, session messages, lore, and settings. Automatic migration or state rewrites continue to use revision checks, backups, or replayable events.
