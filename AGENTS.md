...
- 测试时不需要kill用户的前端进程，也不需要自己启动前端，因为前端是热加载的，只需要打开浏览器访问对应页面就能看到效果变化。
- 测试或验证如果依赖后端变更，优先使用用户已经启动的可见后端，或明确告知用户需要重启；确需由 Codex 重启后端时，必须使用用户可见的前台命令，不允许使用 launchd、nohup、后台守护、自动重拉等不可见方式启动或替换 CaseMagica 前后端进程，也不允许在用户已有项目前后端时私自启动另一套前后端。
- 涉及 Agent 主流程、工具调用、版本恢复或工作区状态的改动，优先补集成测试；纯工具函数再考虑单元测试。
- UI设计倾向于圆角矩形以及高斯模糊，符合 Apple 的设计原则
...
# 前端与交互规范
- 前端改动要遵循 CaseMagica 的 IDE 化工作台气质：主编辑/阅读区域优先，控制项放在稳定 shell 层，避免弹窗和噪音抢占主舞台。
